...
const ALLERGENS = {
  dairy: { label: "Lácteos", className: "allergen-dot--dairy" },
  gluten: { label: "Gluten", className: "allergen-dot--gluten" },
  fish: { label: "Pescado", className: "allergen-dot--fish" },
  eggs: { label: "Huevos", className: "allergen-dot--eggs" },
  soy: { label: "Soja", className: "allergen-dot--soy" },
  peanuts: { label: "Cacahuetes y frutos secos", className: "allergen-dot--peanuts" },
  treeNuts: { label: "Fr. de cáscara", className: "allergen-dot--tree-nuts" },
};
...
const gourmetPizzas = [
  ...
  },
  {
    ...
    image: img4,
    visualLabel: "Favorita de la casa",
  },
  {
    name: "Villa",
    price: "14 €",
    ...
    <div className="menu-card__allergens" aria-label="Alérgenos destacados">
      {items.map((item) => {
        ...
        return (
          ...
          >
            <FaCircle className={allergen.className} />
          </span>
        );
      })}
    </div>
...
function PizzaSectionGrid({ pizzas, matchesFilters, filterMode }) {
  ...
  return (
    <div className="menu-grid">
      {(filterMode === "only" ? visiblePizzas : pizzas).map((pizza) => (
        <div
          key={pizza.name}
          className={`menu-grid__item${pizza.image ? " menu-grid__item--featured" : ""}${matchesFilters(pizza) ? "" : " menu-grid__item--muted"}`}
        >
...
export default function Carta() {
  ...
  const matchesFilters = (pizza) =>
    activeFilters.length === 0 || activeFilters.every((filter) => pizza.allergens?.includes(filter));

  const allPizzas = [...classicPizzas, ...gourmetPizzas];
  const highlightedCount = allPizzas.filter(matchesFilters).length;
  const summaryText =
    activeFilters.length === 0
      ? `Mostrando toda la carta: ${allPizzas.length} pizzas`
      : filterMode === "only"
        ? `Mostrando solo ${highlightedCount} pizzas con los alérgenos seleccionados`
        : `Destacando ${highlightedCount} pizzas con los alérgenos seleccionados`;
    ...
    <section className="menu-page">
      <div className="menu-page__wrap">
        ...
        <section className="menu-page__section">
          ...
          <div className="menu-filters" aria-label="Filtros por alérgenos">
            <p className="menu-filters__summary">
              {activeFilters.length === 0 ? summaryText : (
                <>
                  {filterMode === "only" ? "Mostrando solo " : "Destacando "}
                  <strong>{highlightedCount}</strong>
                  {" "}pizzas con los alérgenos seleccionados
                </>
              )}
            </p>
            ...
            <div className="menu-filter-mode" aria-label="Modo de filtrado">
              <button
                type="button"
                className={`menu-filter-mode__button${filterMode === "highlight" ? " menu-filter-mode__button--active" : ""}`}
                onClick={() => setFilterMode("highlight")}
                aria-pressed={filterMode === "highlight"}
              >
                Destacar coincidencias
              </button>
              <button
                type="button"
                className={`menu-filter-mode__button${filterMode === "only" ? " menu-filter-mode__button--active" : ""}`}
            ...
            {filterItems.map((item) => {
              ...
              return (
                ...
                >
                  <FaCircle className={item.className} />
          ...
          <PizzaSectionGrid pizzas={classicPizzas} matchesFilters={matchesFilters} filterMode={filterMode} />
        </section>
        ...
        <section className="menu-page__section">
          ...
          <PizzaSectionGrid pizzas={gourmetPizzas} matchesFilters={matchesFilters} filterMode={filterMode} />
        </section>
          ...
          <article className="menu-note menu-note--accent menu-note--legend">
            <div className="menu-note__heading">
              <div>
                ...
                <h3>Alérgenos visibles en toda la carta</h3>
              </div>
              <p>Consulta aquí el significado de los puntos de color que aparecen junto al precio de cada pizza.</p>
            </div>
